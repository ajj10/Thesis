...
def fexchangeIcheck(evidence, LRS, players):
    ...
    if(evidence.signer != evidence.message.originator):
        crimes[evidence.signer.name].append("Law #7.1: incorrect originator")
    # check if law 6.2 holds (R in Players)
    if((evidence.message.recevier not in players)):
        crimes[evidence.signer.name].append("Law #7.2: incorrect variable type for recipient")
    ...
    if(type(evidence.message.hash) is not str):
        crimes[evidence.signer.name].append("Law #7.7: ho is not a hash")
    return crimes
...
def abortcheck(evidence, nonces, knownContractsJudge, LRS, players):
    ...
    if(type(evidence.message.me1) is not Sign):
        crimes[evidence.signer.name].append("Law #9.1: me_1 is not signed")
    # check if law 8.1 holds (me_1 is message of format f_exchangeI)
    try:
        if type(evidence.message.me1.message) is not FexchangeI:
            crimes[evidence.signer.name].append("Law #9.1: me_1 is not of format f_exchangeI")
            return crimes
    except:
        crimes[evidence.signer.name].append("Law #9.1: me_1 is not of format f_exchangeI")
        return crimes
    ...
    if(evidence.message.me1.signer != evidence.message.me1.message.originator or evidence.signer != evidence.message.me1.signer):
        crimes[evidence.signer.name].append("Law #9.2: incorrect originator/signer")
    # check if law 8.3 holds (T in LRS)
    if(evidence.message.me1.message.server not in LRS):
        crimes[evidence.signer.name].append("Law #9.3: T not in LRS")
    # check if law 8.4 holds (text is a contract)
    if(type(evidence.message.me1.message.text)!=Contract):
        crimes[evidence.signer.name].append("Law #9.4: text is not a contract")
    # check if law 8.4 holds (text is unique)
    if(evidence.message.me1.message.text in knownContractsJudge):
        crimes[evidence.signer.name].append("Law #9.4: standard contract exists")
    # check if law 8.5 holds (nonce is secret)
    for nonce in nonces:
        if(evidence.message.me1.message.hash == str(h(nonce))):
            crimes[evidence.signer.name].append("Law #9.5: known nonce")
    return crimes
...
def abortedcheck(evidence, nonces, knownContractsJudge, knownContractsT, LRS, players):
    ...
    if type(evidence.abortToken.message.ma1) is not Sign:
        crimes[evidence.abortToken.signer.name].append("Law #10.1: ma_1 is not signed")
    # check if law 9.1 holds (ma_1 is message of format f_abort)
    try:
        if type(evidence.abortToken.message.ma1.message) is not Abort:
            crimes[evidence.abortToken.signer.name].append("Law #10.1: ma_1 is not of format f_abort")
            return crimes
    except:
        crimes[evidence.abortToken.signer.name].append("Law #10.1: ma_1 is not of format f_abort")
        return crimes
    ...
    if(evidence.abortToken.message.ma1.signer != evidence.abortToken.message.ma1.message.me1.signer or evidence.abortToken.message.ma1.message.me1.signer!=evidence.abortToken.message.ma1.message.me1.message.originator):
        crimes[evidence.abortToken.signer.name].append("Law #10.2: incorrect originator/signer")
    # check if law 9.3 holds (T in LRS)
    if(evidence.abortToken.signer not in LRS):
        crimes[evidence.abortToken.signer.name].append("Law #10.3: T not in LRS")
    ...
    if(evidence.abortToken.signer != evidence.abortToken.message.ma1.message.me1.message.server):
        crimes[evidence.abortToken.signer.name].append("Law #10.4: incorrect server")
    # check if law 9.5 holds (text is a contract)
    if(type(evidence.abortToken.message.ma1.message.me1.message.text)!=Contract):
        crimes[evidence.abortToken.signer.name].append("Law #10.5: text is not a contract")
    ...
    if(evidence.abortToken.message.ma1.message.me1.message.text in knownContractsT):
        if(evidence.abortToken.message.ma1.message.me1.message.text.replacement):
            crimes[evidence.abortToken.signer.name].append("Law #10.5: replacement contract exists")
    else:
...
def replacementcheck(evidence, knownContractsT, LRS, players):
    ...
    if type(evidence.replacementContract.message.me1) is not Sign:
        crimes[evidence.replacementContract.signer.name].append("Law #11.1: me_1 is not signed")
    # check if law 10.2 holds (Signed me_2)
    if type(evidence.replacementContract.message.me2) is not Sign:
        crimes[evidence.replacementContract.signer.name].append("Law #11.2: me_2 is not signed")
    ...
    try:
        if type(evidence.replacementContract.message.me1.message) is not FexchangeI:
            crimes[evidence.replacementContract.signer.name].append("Law #11.1: me_1 is not of format f_exchangeI")
            return crimes
        if type(evidence.replacementContract.message.me2.message) is not FexchangeR:
            crimes[evidence.replacementContract.signer.name].append("Law #11.2: me_2 is not of format f_exchangeR")
            return crimes
    except:
        crimes[evidence.replacementContract.signer.name].append("Law #11.1/10.2: me_1/me_2 is of wrong format")
        return crimes
    ...
    if(evidence.replacementContract.signer not in LRS):
        crimes[evidence.replacementContract.signer.name].append("Law #11.3: T not in LRS")
    ...
    if(evidence.replacementContract.signer != evidence.replacementContract.message.me1.message.server):
        crimes[evidence.replacementContract.signer.name].append("Law #11.4: incorrect server")
    ...
    if(evidence.replacementContract.message.me1.signer != evidence.replacementContract.message.me1.message.originator):
        crimes[evidence.replacementContract.signer.name].append("Law #11.5: incorrect originator")
    ...
    if(evidence.replacementContract.message.me2.signer != evidence.replacementContract.message.me1.message.recevier):
        crimes[evidence.replacementContract.signer.name].append("Law #11.6: incorrect recipient")
    # check if law 10.7 holds (text is a contract)
    if(type(evidence.replacementContract.message.me1.message.text) is not Contract):
        crimes[evidence.replacementContract.signer.name].append("Law #11.7: text not a contract")
    ...
    if(evidence.replacementContract.message.me1.message.text in knownContractsT):
        if(evidence.replacementContract.message.me1.message.text.aborted):
            crimes[evidence.replacementContract.signer.name].append("Law #11.7: contract already aborted")
    else:
...
def checkLawBreak(evidence, knownContractsJudge, knownContractsT, LRS, players):
    nonces = []
    crimes= {'O': [], 'R': [], 'T1': [], 'T2': [], 'T3': [], 'X': [], 'Y': [], 'intruder': []}
    if(evidence.req != None):
        server = []
        print("checking request to server:")
        if type(evidence.req.request) is Sign:
            print("checking abort request:")
            server.append(evidence.req.request.message.me1.message.server)
            crimes = abortcheck(evidence.req.request, [], knownContractsJudge, LRS, players)
            print(crimes)
        else:
            server.append(evidence.req.request[0].message.server)
            print("checking me_1 of resolve request:")
            crimes = fexchangeIcheck(evidence.req.request[0], LRS, players)
            print(crimes)
            print("checking me_2 of resolve request:")
            crimes = fexchangeRcheck(evidence.req.request[1], LRS, players)
            print(crimes)
        if evidence.req.answer is not None:
            print("checking answer from T:")
            if type(evidence.req.answer) is not Sign:
                crimes[server[0].name].append("Law #11: Illegal answer from server")
                print(crimes)
            else:
                if type(evidence.req.answer.message) is Aborted:
                    print("checking abort token:")
                    crimes = abortedcheck(evidence, nonces, knownContractsJudge, knownContractsT, LRS, players)
                    print(crimes)
                    print("-------------------------------------")
                elif type(evidence.req.answer.message) is Replacement:
                    print("checking replacement contract:")
                    crimes = replacementcheck(evidence, knownContractsT, LRS, players)
                    print(crimes)
                    print("-------------------------------------")
                else:
                    crimes[evidence.req.answer.signer.name].append("Law #11: Illegal answer from server")
        return
