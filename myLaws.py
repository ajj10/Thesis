...
def law6(state):
    ...
    if(msg.recevier != msg.message.message.recevier):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #6.3: incorrect recipient")
    # check if law 6.4 holds (T in LRS) 
    if(msg.message.message.server not in state.LRS):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #6.4: T not in LRS")
    ...
    if(msg.message.message.text in state.playerContracts[msg.originator.name]):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #6.5: player has singned contract before")
            ...
            for messages in state.playersKnowledge[p.name]:
                if(type(messages)==str and len(messages)>1):
                    if(msg.message.message.hash != messages and msg.message.message.hash == h(messages)):
                        if msg.originator.name == msg.message.signer.name:
                            addToCrimes(msg.message.signer.name, state.crimes, "Law #6.6: known hash")
    # check if law 6.7 holds (hash is a hash)
    if(type(msg.message.message.hash)!=str):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #6.7: h_O is not a hash")
...
def law7(state):
    ...
    if type(msg.message.message.me1) is not Sign:
        addToCrimes(msg.message.signer.name, state.crimes, "Law #7.1: me_1 is not signed")
    # check if law 7.1 holds (me_1 is message of format f_exchangeI)
    if type(msg.message.message.me1.message) is not FexchangeI:
        addToCrimes(msg.message.signer.name, state.crimes, "Law #7.1: me_1 is not of format f_exhangeI")
    ...
    if(msg.message.message.me1.signer != msg.message.message.me1.message.originator):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #7.2: incorrect originator")
    ...
    if(type(msg.message.message.hash)!=str):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #7.7: h_R is not a hash")
...
def law8(state):
    ...
    if(msg.message.message.me1.signer != msg.message.message.me1.message.originator and msg.message.signer!=msg.message.message.me1.signer):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #8.2: incorrect originator/signer of me_1")
    # check if law 8.3 holds (T in LRS)
    if(msg.message.message.me1.message.server not in state.LRS):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #8.3: T not in LRS")
    # check if law 8.4 holds (text is a contract and O not signed it before this protocol run)
    if(type(msg.message.message.me1.message.text)!=Contract):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #8.4: text is not a contract")
    # check if law 8.5 holds (contract is unique)
    if(msg.message.message.me1.message.text in state.knownStandardContracts):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #8.5: same contract with same participants exists")
...
def law9(state):
    ...
    if type(msg.message.message.ma1) is not Sign:
        addToCrimes(msg.message.signer.name, state.crimes, "Law #9.1: ma_1 is not signed")
...
def law10(state):
    ...
    if(type(msg.message.message.me1) != Sign or type(msg.message.message.me2) != Sign):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #10.1/10.2: unsigned me_1/me_2")
    ...
    if(type(msg.message.message.me1.message) != FexchangeI or type(msg.message.message.me2.message) != FexchangeR):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #10.1/10.2: me_1/me_2 is of wrong format")
    ...
    if(msg.message.message.me1.signer != msg.message.message.me1.message.originator):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #10.5: incorrect originator")
    ...
    if(msg.message.message.me2.signer != msg.message.message.me1.message.recevier):
        addToCrimes(msg.message.signer.name, state.crimes, "Law #10.6: incorrect receiver")
